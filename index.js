...
const waitForOrderToExecute = async (tradeConfig, order) => {
    let orderStatus = await exchange.checkOrderStatus(order);
    console.log('start waiting for order to execute...');
    while (orderStatus.status !== 'FILLED') {
        console.log('waiting for order to execute...');
        await delay(tradeConfig.delayDurationMs); //15 minutes
        orderStatus = await exchange.checkOrderStatus(order);
    }
...
const main = async (tradeConfig) => {
    try {
        ...
        const funds = await exchange.getAvailbleFunds();
        ...
        if (hasSufficientFunds(funds.balances, tradeConfig.baseCurrency, tradeConfig.quantity)) {
            ...
            const buyOrder = await exchange.limitBuyOrder(tradeConfig.asset, limitPrice, tradeConfig.quantity);
            ...
            await exchange.limitSellOrder(tradeConfig.asset, limitPrice, tradeConfig.quantity);
        } else {
